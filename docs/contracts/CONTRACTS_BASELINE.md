...
**Status:** Approved documentary baseline

**Date:** 2026-08-18

## 1. Purpose and authority

This document governs the future language-neutral contracts repository for the Loan Onboarding Platform. It defines contract categories, ownership, planned formats, versioning, compatibility, deprecation, validation, and the repository creation gate. The [initial contract catalog](INITIAL_CONTRACT_CATALOG.md) is authoritative for the approved M1 field allowlists.

Domain semantics remain authoritative in the domain sources, bounded contexts retain business ownership, the [Security Model](../architecture/SECURITY_MODEL.md) governs data treatment, and workflows govern cross-capability progression. This baseline does not contain executable OpenAPI, AsyncAPI, or JSON Schema artifacts; it does not assert that `loan-platform-contracts` exists or that any contract has been published. Producers and receiving capabilities retain their business semantics.
...
## 3. Planned standards baseline

| Concern | Planned baseline | Decision |
| --- | --- | --- |
| HTTP APIs | [OpenAPI 3.1.2](https://spec.openapis.org/oas/v3.1.2.html) | Initial target for current ecosystem compatibility. |
| OpenAPI reassessment | [OpenAPI 3.2.0](https://spec.openapis.org/oas/v3.2.0.html) | Official newer version; tooling support must be reevaluated when the repository is created. |
| Async APIs | [AsyncAPI 3.1.0](https://www.asyncapi.com/docs/reference/specification/v3.1.0) | Planned channel, operation, producer, and consumer description. |
| Payload schemas | [JSON Schema Draft 2020-12](https://json-schema.org/draft/2020-12) | Planned envelope, request, event, and policy validation. |
...
These are planned targets, not implemented files.
...
This structure is a plan only and has not been created.
...
With this baseline and [ADR-005](../adr/ADR-005-CONTRACT-GOVERNANCE.md), `loan-platform-contracts` may move to `Ready`.

`Ready` authorizes a separately governed repository-creation task; it does not create the repository, publish a version, start consumer implementation, or make M1 `Demonstrated`. Any new contract or field reopens the same field-level gate for that addition. M1 remains `Defined` until executable contracts and required evidence exist. `Q-008` remains open.
