...
1. Application Process is the persisted saga/process manager. It owns coarse stage, checkpoints, expected facts, correlation/causation, consumed versions, operational/recovery references, deadlines/timer metadata, and idempotency; it does not make identity, credit, document, signature, communication, booking, or disbursement decisions. A replaceable scheduling port activates idempotent due checks; exact scheduler technology remains deferred to runtime/IaC design. See [ADR-003](../adr/ADR-003-EVENT-DRIVEN.md).
...
| Application Process | Persist and coordinate coarse process state, checkpoints, expected facts, correlation/causation, recovery, deadlines/timer metadata, and idempotency without deciding capability outcomes. | Application Process | In: public commands, integration events, and due-check signals through a replaceable scheduling port. Out: integration events and capability requests through ports. | Application and onboarding-process state, Inbox/Outbox/idempotency records; no capability-owned decision or evidence. | Event transport, owned store, replaceable scheduler port. | Local; initial AWS walking skeleton. | Confirmed boundary; Planned implementation |
