...
| Confirmed transfer | DS records success through existing owner path; LB continues `ConfirmDisbursement`/`ActivateLoanAccount` flow | Matching amount, currency, reservation, provider result | Manually manufacture success or mutate LB data | Normal activation path |
