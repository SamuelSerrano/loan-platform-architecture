...
| BR-AP-007 | One active application per customer | Confirmed | Application Process | Customer ID, active Credit Applications | A customer cannot have more than one active application in MVP | Continue the existing application or reject creation of another active application | `ACTIVE_APPLICATION_ALREADY_EXISTS` | [Demonstration product](PRODUCT_AND_CREDIT_POLICY.md#2-demonstration-product) |
...
| BR-CI-003 | Identity verification records validity and provider-neutral strength | Confirmed | Customer & Identity | Verification outcome, verification level, validity instant | A successful verification must record the achieved provider-neutral level and its validity instant when applicable | Publish a provider-neutral verified outcome or a rejected/incomplete outcome with safe reason codes | `IDENTITY_NOT_VERIFIED`; `IDENTITY_EVIDENCE_INSUFFICIENT` | [Customer & Identity vocabulary](UBIQUITOUS_LANGUAGE.md#6-customer--identity-vocabulary) |
...
| BR-CD-004 | A favorable decision requires eligible alternatives | Confirmed | Credit Decisioning | Completed guard evaluations, calculated alternatives | All guards must pass and at least one distinct eligible alternative must exist | Record `Favorable` with the alternatives allowed by the specialized decision table; otherwise apply the relevant terminal rule | `ELIGIBLE_AMOUNT_BELOW_PRODUCT_MINIMUM` when none qualifies | [Terminal guard decision table](CREDIT_DECISION_TABLE.md#5-terminal-guard-decision-table) |
| BR-CD-005 | Offer construction obeys all governing limits | Confirmed | Credit Decisioning | Requested amount, affordability result, risk cap, segment cap, product limit, permitted terms | Every alternative must remain within every applicable constraint and installment capacity | Return distinct alternatives under the normative calculation, treatment matrices, and specialized cardinality limit | See normative guard or limiting-cap reason | [Affordability and offer calculation](CREDIT_DECISION_TABLE.md#8-affordability-and-offer-calculation) |
...
| BR-CD-008 | Credit eligibility requires valid unexpired identity verification | Confirmed | Credit Decisioning | Provider-neutral identity outcome, verification level, validity instant, assessment instant | A favorable decision is prohibited when identity verification failed, expired, or confirms a data mismatch | Produce the decision-table outcome without changing the identity-owned verification record | `IDENTITY_NOT_VERIFIED` | [Terminal guard decision table](CREDIT_DECISION_TABLE.md#5-terminal-guard-decision-table) |
| BR-CD-009 | An active loan prevents a new favorable assessment in MVP | Confirmed | Credit Decisioning | Customer ID, active-loan indicator | A customer with an Active Loan cannot pass the concurrent-exposure guard in MVP | Produce `Unfavorable` under the normative guard | `ACTIVE_LOAN_ALREADY_EXISTS` | [Terminal guard decision table](CREDIT_DECISION_TABLE.md#5-terminal-guard-decision-table) |
...
| BR-DP-005 | Correction or accepted-term change invalidates the obsolete package | Confirmed | Document Preparation | Existing Document Package, correction or changed accepted terms | A package generated from superseded contents or terms cannot remain eligible for approval or signing | Invalidate the obsolete package and generate a new Package Version when progression continues | — | [Aggregate invariants — DocumentPackage and SignatureEnvelope](BOUNDED_CONTEXTS_AND_EVENT_STORMING.md#documentpackage-and-signatureenvelope) |
...
| BR-ES-005 | Package invalidation makes its bound envelope unusable | Confirmed | Electronic Signature | Signature Envelope, bound Package Version, package-invalidated fact | An envelope bound to a corrected, changed, or otherwise invalidated package cannot authorize signature | Invalidate the envelope and require a new envelope for the replacement Package Version | — | [Document Preparation vocabulary](UBIQUITOUS_LANGUAGE.md#8-document-preparation-vocabulary) |
...
| BR-DS-004 | Destination data is tokenized or simulated | Confirmed | Disbursement | Disbursement Destination | Raw bank credentials cannot be stored or propagated | Use a validated token or masked/simulated destination reference | — | [Disbursement vocabulary](UBIQUITOUS_LANGUAGE.md#12-disbursement-vocabulary) |
...
| BR-XS-004 | Consumers handle at-least-once delivery idempotently | Derived | Cross-cutting | Event ID, aggregate version, Inbox record | Duplicate or out-of-order delivery cannot repeat the business effect or regress state | Ignore safely, retain deduplication evidence, and record an operational metric | — | [Failure and compensation paths](EVENT_STORMING.md#9-alternate-and-recovery-paths) |
| BR-XS-005 | Audit cannot control the synchronous workflow | Confirmed | Cross-cutting | Business/security fact, Audit Trail projection | Audit consumption cannot be a synchronous dependency of a business transition | Append or project the sanitized fact asynchronously | — | [Context relationships](BOUNDED_CONTEXTS_AND_EVENT_STORMING.md#6-context-relationships) |
