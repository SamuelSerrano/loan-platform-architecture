...
This catalog does not define transport schemas. Future AsyncAPI and JSON Schema contracts belong to `loan-platform-contracts`. The [bounded-context map](BOUNDED_CONTEXTS_AND_EVENT_STORMING.md#4-bounded-context-map) remains authoritative for ownership and relationships, while the [Credit Decisioning design](CREDIT_DECISIONING_DESIGN.md#11-domain-events) remains authoritative for that context's internal implementation design.
...
| IE-CI-002 | `CustomerIdentityRejected.v1` | Proposed | Customer & Identity | Application Process; Communications; Audit Trail projection | `CustomerIdentityRejected` commits | Event envelope; application and customer IDs; verification ID; safe reason codes; rejected instant | Raw identity evidence; full PII; restricted fraud details; provider credentials | Initial public contract; incompatible change requires `.v2` | [BR-CI-003](BUSINESS_RULES.md#5-customer--identity-ci), [BR-CI-004](BUSINESS_RULES.md#5-customer--identity-ci), [BR-XS-002](BUSINESS_RULES.md#12-cross-cutting-xs) | [Executive journey and event authority](BOUNDED_CONTEXTS_AND_EVENT_STORMING.md#7-executive-journey-summary) |
