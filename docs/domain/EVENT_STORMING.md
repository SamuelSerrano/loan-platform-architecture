...
| Administrator | Human actor | Performs authorized operational recovery and reconciliation; has no implicit underwriting authority | [Open question Q-009](../discovery/ASSUMPTIONS.md#open-questions) |
| Identity Provider | External business system | Supplies provider-specific verification evidence through the Customer & Identity anti-corruption layer | [Context relationships](BOUNDED_CONTEXTS_AND_EVENT_STORMING.md#5-context-relationships) |
...
| Amazon Cognito | Generic platform system | Authenticates actors and supplies authorization scopes; it owns no business decision | [Domain classification](BOUNDED_CONTEXTS_AND_EVENT_STORMING.md#2-domain-classification) |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| A1 | Applicant | CMD-AP-001 `StartCreditApplication` | `CreditApplication` | [DE-AP-001](DOMAIN_EVENTS.md#41-application-process-ap) | Capture progressive customer data | — | Applicant Application Tracker | Confirmed |
| A2 | Applicant | CMD-CI-001 `RegisterCustomerProfile` | `Customer` | [DE-CI-001](DOMAIN_EVENTS.md#42-customer--identity-ci) | Request the required purpose-bound consents | — | Customer profile view | Confirmed |
| A3 | Applicant | CMD-CI-002 `GrantConsent` | `ConsentRecord` | [DE-CI-002](DOMAIN_EVENTS.md#42-customer--identity-ci) | Allow submission only when [BR-CI-001](BUSINESS_RULES.md#5-customer--identity-ci) and the required consent set hold | — | Consent status | Confirmed |
| A4 | Applicant | CMD-AP-002 `SubmitCreditApplication` | `CreditApplication` | [DE-AP-002](DOMAIN_EVENTS.md#41-application-process-ap) | POL-AP-001 start identity verification after [BR-AP-001](BUSINESS_RULES.md#4-application-process-ap) succeeds | [IE-AP-001](DOMAIN_EVENTS.md#51-application-process-ap) | Customer & Identity; Audit Trail projection | Proposed public mapping |
| A5 | Application-submitted reaction | CMD-CI-003 `StartIdentityVerification` | `IdentityVerification` | [DE-CI-003](DOMAIN_EVENTS.md#42-customer--identity-ci) | Invoke the provider through the owning anti-corruption layer | — | Identity Provider | Confirmed |
| A6 | Provider-neutral successful result | CMD-CI-004 `RecordIdentityVerification` | `IdentityVerification` | [DE-CI-004](DOMAIN_EVENTS.md#42-customer--identity-ci) | POL-AP-002 request assessment only while verification satisfies [BR-CD-008](BUSINESS_RULES.md#6-credit-decisioning-cd) | [IE-CI-001](DOMAIN_EVENTS.md#52-customer--identity-ci) | Application Process; Audit Trail projection | Proposed public mapping |

## 5. Phase B — Decision and offer

| Step | Actor / trigger | Command | Aggregate | Domain event | Policy | Integration event | Consumer | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| B1 | Verified-identity reaction | CMD-CD-001 `RequestCreditAssessment` | `CreditAssessment` | [DE-CD-001](DOMAIN_EVENTS.md#43-credit-decisioning-cd) | Evaluate the immutable snapshot under [BR-CD-001](BUSINESS_RULES.md#6-credit-decisioning-cd) and deterministic precedence | — | Credit Decisioning | Confirmed |
...
| B3 | Favorable-decision reaction | CMD-CD-003 `CalculateOfferAlternatives` | `CreditAssessment` | [DE-CD-004](DOMAIN_EVENTS.md#43-credit-decisioning-cd) | Present eligible alternatives without moving ownership to the coordinator | — | Applicant offer-options view | Confirmed |
...
| B5 | Applicant | CMD-AP-003 `AcceptCreditOffer` | `CreditApplication` (ownership unresolved; see hotspot) | [DE-AP-003](DOMAIN_EVENTS.md#41-application-process-ap) | POL-AP-004 request document generation after exact-terms acceptance | [IE-AP-002](DOMAIN_EVENTS.md#51-application-process-ap) | Document Preparation; Audit Trail projection | Proposed |

The acceptance command, aggregate attribution, private fact, public producer, and reaction in B5 deliberately preserve the unresolved ownership discrepancy; they are not an adopted resolution.

## 6. Phase C — Documents and approval

| Step | Actor / trigger | Command | Aggregate | Domain event | Policy | Integration event | Consumer | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| C1 | Accepted-offer reaction | CMD-DP-001 `GenerateDocumentPackage` | `DocumentPackage` | [DE-DP-001](DOMAIN_EVENTS.md#44-document-preparation-dp) | Make references and hashes available; bytes remain protected under [BR-DP-004](BUSINESS_RULES.md#7-document-preparation-dp) | [IE-DP-001](DOMAIN_EVENTS.md#54-document-preparation-dp) | Application Process; Audit Trail projection | Proposed public mapping |
...
| C3 | Approved-package reaction | CMD-ES-001 `CreateSignatureEnvelope` | `SignatureEnvelope` | [DE-ES-001](DOMAIN_EVENTS.md#45-electronic-signature-es) | Request a purpose-, signer-, and envelope-bound challenge | — | Electronic Signature | Confirmed |
...
| D2 | Channel adapter success | CMD-CO-002 `RecordOtpDelivery` | `NotificationDelivery` | [DE-CO-002](DOMAIN_EVENTS.md#46-communications-co) | Preserve delivery separately from validation under [BR-CO-004](BUSINESS_RULES.md#9-communications-co) | — | Signature status view | Confirmed |
...
| Step | Actor / trigger | Command | Aggregate | Domain event | Policy | Integration event | Consumer | Status |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| E1 | Signed-package reaction | CMD-LB-001 `ReserveLoanAccount` | `LoanAccount` | [DE-LB-001](DOMAIN_EVENTS.md#47-loan-booking-lb) | POL-DS-001 create one transfer order for the pending reservation | [IE-LB-001](DOMAIN_EVENTS.md#57-loan-booking-lb) | Disbursement; Application Process; Audit Trail projection | Proposed public mapping |
| E2 | Reservation reaction | CMD-DS-001 `CreateDisbursementOrder` | `DisbursementOrder` | [DE-DS-001](DOMAIN_EVENTS.md#48-disbursement-ds) | Validate the tokenized destination under [BR-DS-004](BUSINESS_RULES.md#11-disbursement-ds) | — | Disbursement | Confirmed |
| E3 | Valid order | CMD-DS-002 `ExecuteDisbursement` | `DisbursementOrder` | [DE-DS-002](DOMAIN_EVENTS.md#48-disbursement-ds) | Call the provider once under the stable identity required by [BR-DS-002](BUSINESS_RULES.md#11-disbursement-ds) | — | Disbursement Provider | Confirmed |
| E4 | Provider confirmation | CMD-DS-003 `ConfirmDisbursement` | `DisbursementOrder` | [DE-DS-003](DOMAIN_EVENTS.md#48-disbursement-ds) | POL-LB-002 activate only when amount, currency, and reservation reference match | [IE-DS-001](DOMAIN_EVENTS.md#58-disbursement-ds) | Loan Booking; Application Process; Communications; Audit Trail projection | Proposed public mapping |
| E5 | Confirmed-funds reaction | CMD-LB-002 `ActivateLoanAccount` | `LoanAccount` | [DE-LB-002](DOMAIN_EVENTS.md#47-loan-booking-lb) | POL-AP-005 complete the matching journey; only now does Customer become Borrower | [IE-LB-002](DOMAIN_EVENTS.md#57-loan-booking-lb) | Application Process; Communications; Audit Trail projection | Proposed public mapping |
| E6 | Activated-loan reaction | CMD-AP-004 `CompleteCreditApplication` | `CreditApplication` | [DE-AP-005](DOMAIN_EVENTS.md#41-application-process-ap) | Send completion notification asynchronously | [IE-AP-004](DOMAIN_EVENTS.md#51-application-process-ap) | Communications; Audit Trail projection | Proposed public mapping |
...
| Identity provider unavailable | Customer & Identity | Timeout or temporary provider failure; no completed identity outcome | Retry asynchronously, then isolate exhausted delivery/work for controlled review | Publish verified/rejected by inference or start assessment | Preserve verification identity; query ambiguous provider results before retry | `IdentityPending` |
...
| Offer rejected | Applicant / ownership unresolved | CMD-AP-005 `RejectCreditOffer` intends explicit rejection | POL-AP-009 close the offer path after the proposed [DE-AP-004](DOMAIN_EVENTS.md#41-application-process-ap) | Present AP ownership as settled or generate documents | Redelivery cannot repeat the rejection effect | `OfferClosed`; proposed [IE-AP-003](DOMAIN_EVENTS.md#51-application-process-ap) |
| Offer expired | Credit Decisioning | [DE-CD-006](DOMAIN_EVENTS.md#43-credit-decisioning-cd) | POL-AP-010 prevent acceptance and require an explicit reassessment/new-offer policy | Accept stale terms or silently recalculate a decision | Clock processing and redelivery are idempotent | `OfferExpired`; [IE-CD-007](DOMAIN_EVENTS.md#53-credit-decisioning-cd) |
...
| Disbursement terminal failure | Disbursement | Non-retryable transfer failure | POL-LB-003 stop activation and invoke explicit reservation recovery | Create an Active Loan or reinterpret failure as decline | Duplicate terminal facts are harmless | `DisbursementFailed`; proposed [IE-DS-002](DOMAIN_EVENTS.md#58-disbursement-ds) |
| Disbursement outcome unknown | Disbursement | [DE-DS-005](DOMAIN_EVENTS.md#48-disbursement-ds) after ambiguous submission | CMD-DS-004 `ReconcileDisbursementOutcome` against the provider's authoritative result | Retry, fail, cancel, or activate before reconciliation | Query by stable order/key; do not submit a new transfer | `DisbursementPending` until known |
| Activation failure after confirmed funds | Loan Booking | Confirmed transfer cannot be applied to the pending reservation | POL-LB-004 enter critical reconciliation and retry activation | Automatically reverse funds or label the application declined | Activation command is idempotent; preserve matching confirmation | `ActivationPending` critical under [BR-LB-005](BUSINESS_RULES.md#10-loan-booking-lb) |
...
| Manual recovery | Administrator plus owning context | A diagnosed case requires an operation not safely automated | CMD-XS-002 `RequestManualRecovery` with authority, evidence, and correlation reference | Direct database mutation, silent distributed rollback, or invented underwriting override | Recovery command must be unique, traceable, and context-owned | Explicit recovery or unresolved case; permitted action remains open |
...
| --- | --- | --- | --- | --- | --- | --- |
| HS-001 | Proposed | Application Process and Credit Decisioning | Offer acceptance/rejection ownership | Determines the command aggregate, private fact producer, public producer, and invariants for progression to documents | Resolve whether Application Process records the applicant action or `CreditOffer` owns it; until then acceptance/rejection remain proposed | [Explicit ownership note](DOMAIN_EVENTS.md#49-cross-cutting-xs), [P-005](../discovery/ASSUMPTIONS.md#confirmed-product-decisions) |
...
| HS-003 | Assumption | Platform architecture | EventBridge, SQS, Outbox, and Inbox adequacy | Affects delivery isolation, cost, ordering, and operational recovery | Validate delivery and persistence assumptions under load and failure | [H-004 and H-005](../discovery/ASSUMPTIONS.md#working-assumptions-requiring-later-validation) |
| HS-004 | Open | Disbursement and operations | Recovery after repeated transfer failure | Determines reservation cancellation/expiry and operator authority | Define allowed manual recovery without implying automatic reversal | [Q-009](../discovery/ASSUMPTIONS.md#open-questions) |
| HS-005 | Open | Security and contract governance | Sensitive-field classification and retention | Affects public payload allowlists, storage, logs, audit, and DLQs | Classify PII/tokenization and set retention before publication/IaC | [Q-003 and Q-004](../discovery/ASSUMPTIONS.md#open-questions) |

## 12. Open questions

| ID | Status | Owner / decision maker | Topic | Impact | Decision required | Source |
| --- | --- | --- | --- | --- | --- | --- |
| OQ-001 | Open | Application Process architecture | Saga persistence and timers | Determines persisted coordinator state, timeout commands, and replay behavior | Choose choreography-only policies or explicit saga steps/timers | [Q-005](../discovery/ASSUMPTIONS.md#open-questions) |
| OQ-002 | Open | Customer & Identity | Identity validity across reassessment | Determines whether a new assessment may reuse a verification | Define exact expiration and renewal policy | [Q-006](../discovery/ASSUMPTIONS.md#open-questions) |
| OQ-003 | Open | Loan Booking | Schedule dates and rounding | Blocks normative schedule generation and contract examples | Define first-installment date and rounding conventions | [Q-007](../discovery/ASSUMPTIONS.md#open-questions) |
| OQ-004 | Open | Product, Communications, and UI governance | Customer-safe reason disclosure | Affects decline, identity, and failure messages | Classify codes that may be shown directly versus translated | [Q-008](../discovery/ASSUMPTIONS.md#open-questions) |
| OQ-005 | Open | Operations and Disbursement | Repeated-failure manual action | Keeps terminal transfer recovery deliberately unresolved | Define authorized action, evidence, and audit requirements | [Q-009](../discovery/ASSUMPTIONS.md#open-questions) |
| OQ-006 | Open | Security and platform architecture | Retention and DLQ treatment | Affects protected artifacts, OTP records, audit evidence, and redrive | Set retention, encryption, deletion, and access rules | [Q-003](../discovery/ASSUMPTIONS.md#open-questions) |
