...
| B5 | Applicant | CMD-AP-003 `AcceptCreditOffer` | `CreditApplication` | [DE-AP-003](DOMAIN_EVENTS.md#41-application-process-ap) | POL-AP-004 verify the active offer reference and exact canonical `termsHash`, record the idempotent applicant action and stage transition, then request document generation | [IE-AP-002](DOMAIN_EVENTS.md#51-application-process-ap) | Document Preparation; Audit Trail projection; Credit Decisioning projection if required | Confirmed private ownership; public event Proposed |
