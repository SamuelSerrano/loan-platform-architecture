...
| ID | Status | Owner / decision maker | Topic | Impact | Decision required | Source |
| --- | --- | --- | --- | --- | --- | --- |
| OQ-001 | Confirmed | Application Process architecture | Saga persistence and timers | Establishes restart-safe coarse coordination, deadlines, due checks, and replay behavior without moving capability decisions | Resolved 2026-08-14: persist coarse saga/process state, checkpoints, expected facts, correlation/causation, consumed versions, operational/recovery references, deadline/timer metadata, and idempotency; activate due checks through a replaceable scheduling port | [ADR-003](../adr/ADR-003-EVENT-DRIVEN.md#persisted-deadlines-and-timers--q-005-resolution) |
