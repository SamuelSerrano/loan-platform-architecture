...
- The contracts repository cannot become `Ready`, and versioned contracts cannot be published, until issue #8 resolves `Q-004` for the initial contracts and satisfies the repository gate.
- No implementation may consume the initial shared contracts before that `Q-004` resolution and publication evidence exists.
...
| `Q-004` | Remains open in issue #7. Issue #8 must resolve it for the initial contract field allowlists before the contracts repository becomes `Ready`, versioned contracts are published, M1 becomes `Demonstrated`, or implementations consume those contracts. |
...
Issue #8 retains exclusive responsibility for executable contract fields, payloads, schemas, detailed envelopes and metadata, normative names beyond existing canonical sources, versioning, compatibility, deprecation, and field-by-field allowlists. Issue #8 must resolve `Q-004` for the initial contracts before the related readiness, publication, M1 demonstration, and consumer-implementation gates can pass. This roadmap does not close `Q-004`; it defines only the contracts repository's purpose, scope, dependencies, entry criteria, first slice, milestone, creation order, and required evidence.
